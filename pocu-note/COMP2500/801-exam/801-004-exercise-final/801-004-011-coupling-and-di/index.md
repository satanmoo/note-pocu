...
a) A가 B에 의존하는 상황에서, B를 변경했을 때 A의 내부 코드까지 수정해야만 프로그램이 제대로 동작하는 상태를 결합도가 높다(tight coupling)고 함. 즉 결합도는 의존 대상의 변경이 나에게 전파되는 정도임. 의존성 자체는 기능이 잘 분리됐다는 뜻이라 좋은 설계이고, 나쁜 것은 높은 결합도임. 예를 들어 A가 내부에서 `new B()`로 개체를 직접 생성하면, B의 생성자가 바뀔 때 A의 코드도 고쳐야 하므로 결합도가 높음.

b) 최소 2개:
1. ==의존성 주입(생성자 주입)== — 의존하는 개체를 외부에서 생성해 생성자의 매개변수로 전달. 내가 상대의 생성(내부 구현)에 관여하지 않게 되어 상대의 생성자·구현 변경에 영향을 안 받음
2. ==부모(일반화된) 타입·인터페이스에 의존== — 멤버 변수·매개변수를 구체 클래스 대신 부모 타입으로 선언하고 자식 개체를 주입. 다형성 덕분에 구체 클래스를 교체해도 내 코드는 무변경
